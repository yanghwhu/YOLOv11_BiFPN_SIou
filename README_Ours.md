...
## Installation (The operating device must have a graphics card capable of utilising the GPU, with recommended video memory exceeding 4GB.)

### Prerequisites (Preparations before you begin)
- Python 3.8+
- Install PyCharm (For easy viewing of the code)  
     (https://www.jetbrains.com/pycharm/download/?section=windows#section=windows)
- Install Anaconda3
     Download link: https://repo.anaconda.com/miniconda/  
     (Recommended download: Miniconda3-py39_24.9.2-0-Windows-x86_64, During installation, you must tick the box for ‘Add Anaconda to my PATH environment variable’.)
- Install CUDA  (for GPU support, Need to select the appropriate version based on your graphics card.) 
     (Install graphics card drivers and CUDA: https://developer.nvidia.com/cuda-toolkit-archive, https://developer.nvidia.com/cudnn)
### Quick Start Guide
Steps 1. **Download the stone burial site deep learning dataset **:
   ```sh
   ...
   # Note that the datasets folder directly contains images and labels.

Steps 2. **Clone the repository**:
   ```sh
   git clone https://github.com/yanghwhu/YOLOv11_BiFPN_SIou.git
   cd YOLOv8_BiFPN_SIou
   ```

### if you have already set up or can set up the environment required for the YOLOv11 model, please refer to the steps in README_Ours; 
### if not, or if you are working cross-platform, please refer to the Dockerfile and execute it within a Docker container.

Steps 3. **Setting up a PyTorch environment**:
   ```sh
     conda create -n YOLOv11 python=3.9
   ```
Steps 4. **Activate the created environment**:
  ```sh
   conda activate YOLOv11
  ```
Steps 5. **Install dependencies**:
 ```sh
    pip3 install torch torchvision --index-url https://download.pytorch.org/whl/cu128
    # The CUDA version must be compatible with your current device.
    #Mac OS system users may refer to the following URL: https://pytorch.org/get-started/previous-versions/
  ```
Steps 6. **Install YOLOv11**:
  ```sh
    pip install ultralytics
  ```
Steps 7. **To train the YOLOv11_BiFPN_SIou model**:
 ```sh
...
Steps 8. **Results**:
  ```sh
   Training results are saved in "runs/train/exp"
  ```
...
## Contact Information
GitHub: https://github.com/yanghwhu/YOLOv11_BiFPN_SIou.git
Email: yanghjlu@163.com
QQ: 919462483