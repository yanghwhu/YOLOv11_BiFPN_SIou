...
def test_val():
    ...
    YOLO(MODEL).val(data="tomb.yaml", imgsz=32, save_hybrid=True)


def test_train_scratch():
    """Test training the YOLO model from scratch using the provided configuration."""
    model = YOLO(CFG)
    model.train(data="tomb.yaml", epochs=2, imgsz=32, cache="disk", batch=-1, close_mosaic=1, name="model")
    model(SOURCE)
...
def test_workflow():
    ...
    model = YOLO(MODEL)
    model.train(data="tomb.yaml", epochs=1, imgsz=32, optimizer="SGD")
    model.val(imgsz=32)
...
def test_yolov10():
    ...
    # train/val/predict
    model.train(data="tomb.yaml", epochs=1, imgsz=32, close_mosaic=1, cache="disk")
    model.val(data="tomb.yaml", imgsz=32)
